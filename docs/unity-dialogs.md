...
| "The open scene(s) have been modified externally": "The following open scene(s) have been changed on disk: … Do you want to reload the scene(s)?" (`EditorSceneManager.cpp`) | "Reload" / "Ignore" | **Auto: Reload, only when the scenes are known clean**; otherwise notify | Raised when a branch switch rewrites the file of a scene open in the editor. Reload discards unsaved in-editor scene edits, so it is pressed only within minutes of the app's own `switch_branch` checking over the MCP bridge that no open scene was dirty, and never when the editor's title has a `*`. A worker running `git switch` itself gives no such check, so that case is reported. |
...
| "Recovering Scene Backups": "Scene backups from a previous Editor session have been detected…" | not confirmed in the binary | Notify | It is about keeping lost scene edits; the button labels could not be verified. |
